...
from gru.numpy_impl import init_parameters, sequence_forward, step_forward
...
def test_init_shapes_dtype_and_reproducibility() -> None:
    ...
    assert first["weight_ih"].shape == (15, 3)
    assert first["weight_hh"].shape == (15, 5)
    assert first["bias_ih"].shape == first["bias_hh"].shape == (15,)
    for name in first:
        assert first[name].dtype == np.float64
        np.testing.assert_array_equal(first[name], second[name])


def test_step_matches_official_gru_cell() -> None:
    ...
    official = torch.nn.GRUCell(3, 2).double()
    with torch.no_grad():
        for name, parameter in official.named_parameters():
            parameter.copy_(torch.from_numpy(params[name]))
    expected = official(torch.from_numpy(x), torch.from_numpy(h))
    np.testing.assert_allclose(actual, expected.detach().numpy(), rtol=1e-10, atol=1e-10)
