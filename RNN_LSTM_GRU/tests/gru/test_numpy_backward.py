...
from gru.numpy_impl import init_parameters, sequence_backward, sequence_forward
...
def test_sequence_backward_matches_finite_differences() -> None:
    ...
    def objective() -> float:
        current_outputs, current_h, _ = sequence_forward(x, h0, params)
        return float(
            np.sum(current_outputs * doutputs) + np.sum(current_h * dh_last)
        )
